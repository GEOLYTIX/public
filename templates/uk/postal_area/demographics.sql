        ...
        from 
            (SELECT 
                ...
                from geodata.uk_glx_geodata_oa_metrics m
                    join geodata.uk_glx_geodata_postal_area s on  st_intersects(st_transform(s.geom_3857_50m,4326), m.geom_p_4326)
                where 
                    s.id = ${id}
			) o
            cross join (
                ...
                from geodata.uk_glx_geodata_workers_hybrid_zone w
                    join geodata.uk_glx_geodata_postal_area s on  st_intersects(s.geom_3857_50m, w.geom_p_3857)
                where 
                    s.id = ${id} 
            ) w
         )site
